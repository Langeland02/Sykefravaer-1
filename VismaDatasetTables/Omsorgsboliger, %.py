...
def omsorgsboliger():
    ...
    for year in years:
        for avdeling in unike_avdelinger:
            ...
            deviation_codes = {"egenmeldinger": "EM", 'Sykemeldinger (over 16 dager)': 'SO',
                               'Sykemeldinger (under 17 dager)': 'SU', 'Overtid': 'OT', 'Ekstravakt': 'EV'}
            for case, code in deviation_codes.items():
                num_obs = avd[avd['DeviationTypeShortName'] == code].shape[0]
                condition_individuals = avd[avd['DeviationTypeShortName'] == code]['FirstName'].nunique()
                if num_individuals > 0:
                    prosentandel = (num_obs / avd.shape[0]) * 100
                    summarize_day_month_counts(avd, 'dayName', 'monthName', 'DeviationTypeShortName', code,
                                               f'Prosentandel {case} {year} :  {avdeling}',
                                               f"Datasettet er hentet fra visma. I systemet er {case} registrert som avvik med avvikskoden '{code}'. Tallene i koordinatsystemet ovenfor viser prosentandelen {case} registrert i løpet av måneden, fordelt på hvilken ukedag tilfellene fant sted. Avdelingen '{avdeling}' hadde totalt {num_individuals} registrerte ansatte. Det funnet {case} for {condition_individuals} ({int(round(condition_individuals / num_individuals, 2) * 100)}%) av de ansatte i løpet av 2023. Totalt er det observert {num_obs} avvik med koden '{code}', som utgjør {prosentandel:.2f}% av total grunnturnus.",
                                               output_filename=f'tabels/omsorgsboliger/{case}_{avdeling}_{year}_ny.xlsx")
                    else:
                    summarize_day_month_counts(avd, 'dayName', 'monthName', 'DeviationTypeShortName', code,
                                               f'Prosentandel {case} {year} :  {avdeling}',
                                               f"Datasettet er hentet fra visma. I systemet er {case} registrert som avvik med avvikskoden '{code}'. Tallene i koordinatsystemet ovenfor viser prosentandelen {case} registrert i løpet av måneden, fordelt på hvilken ukedag tilfellene fant sted. Avdelingen '{avdeling}' hadde totalt {num_individuals} registrerte ansatte. Det funnet {case} for {condition_individuals} av de ansatte i løpet av 2023. Totalt er det observert {num_obs} avvik med koden '{code}', som utgjør {prosentandel:.2f}% av total grunnturnus.",
